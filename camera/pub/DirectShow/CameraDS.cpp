...
void CCameraDS::SetFlip()
{
	
	m_fliptag = true;
}
void CCameraDS::SetUnFlip()
{
	m_fliptag = false;
}
...
bool CCameraDS::SaveBmpFile(char *path, CPIC_RGB *target)
{
...
void CCameraDS::QueryFrame(CPIC_RGB * m_retp)
{
	if(m_retp==NULL)
	{
		return ;
	...
	m_retp->init(m_pFrame->width,m_pFrame->height);
	m_retp->SetBitBYTE(m_pFrame->width,m_pFrame->height,m_pFrame->imageDataOrigin);
}
